...
function getConfig(): Config {
  ...
  return {
    ...
    startHealthServer:
      String(process.env.START_HEALTH_SERVER ?? "true") === "true",
...
function getEffectiveCursorIso(lookbackStartIso: string): string {
  ...
  if (!Number.isFinite(cursorMs) || cursorMs < lookbackMs)
    return lookbackStartIso;
  return cursorIso;
}
...
async function maybeClickButtonByText(
  currentPage: Page,
  textMatcher: string
): Promise<boolean> {
...
async function settleLanguageAndTermsDialogs(currentPage: Page): Promise<void> {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const clickedLanguage = await maybeClickButtonByText(
      currentPage,
      "english"
    );
    if (clickedLanguage) {
      ...
      const hasPendingDialog = await currentPage
        .evaluate(() => {
          ...
          return (
            ...
            bodyText.includes("select language")
          );
        })
        .catch(() => false);
...
async function fetchReportFeedItems(
  currentPage: Page,
  ...
  const response = await currentPage.evaluate(async (sinceIso) => {
    ...
    const contentType =
      feedResponse.headers.get("content-type")?.toLowerCase() ?? "";
  ...
  if (!response.contentType.includes("msgpack")) {
    console.warn(
      `[listener] report-feed non-msgpack response: ${
...
async function ensureConnectedPage(): Promise<Page> {
  ...
  await Promise.race([
    ...
    page
      .waitForFunction(
        () => document.body?.innerText?.includes("People Over Papers") ?? false,
        {
          timeout: 20_000,
        }
      )
  ...
  await Promise.race([
    page
      .waitForSelector(".mat-mdc-tab-body-content", { timeout: 12_000 })
      .catch(() => null),
    sleep(2_500),
  ]);
  ...
  return currentPage.evaluate(
    async ({
      ...
      const summaryById = new Map<
        string,
        { sourceCreatedAt: string; summary: Record<string, unknown> }
      >();
      ...
      function normalizeSummaryCandidate(
        item: unknown
      ): {
        sourceId: string;
        sourceCreatedAt: string;
        ...
        if (sourceIdCandidate === undefined || sourceIdCandidate === null)
          return null;
      ...
      function upsertSummary(item: unknown): boolean {
        ...
        if (
          !existing ||
          Date.parse(candidate.sourceCreatedAt) >=
            Date.parse(existing.sourceCreatedAt)
        ) {
      ...
      async function fetchJson(
        ...
        if (!contentType.includes("application/json") || looksLikeHtml) {
          return {
            ...
            json: null,
            text: `Non-JSON response for ${url} (${
      ...
      function extractItems(payload: unknown): {
        ...
        if (payload && typeof payload === "object") {
          ...
          const items = Array.isArray(objectPayload.results)
            ...
            : [];
          const next =
            typeof objectPayload.next === "string" ? objectPayload.next : null;
          return {
            items,
            next,
          };
      ...
      async function fetchReportDetail(
        ...
        const reportUrls = [
          ...
          `/api/reports/${encodedId}/?archived=False&incident_time__gte=${encodeURIComponent(
            lookbackStartIso
          )}&incident_time__lte=${encodeURIComponent(nowIso)}`,
        ];
      ...
      let reportsUrl = `/api/reports/?archived=False&page=1&page_size=${encodeURIComponent(
        pageSize
      )}`;
      ...
      const rows: ScrapeRow[] = reportIds
        ...
        .sort(
          (left, right) =>
            Date.parse(right.sourceCreatedAt) - Date.parse(left.sourceCreatedAt)
        );

      const newestCreatedAt = rows.length ? rows[0].sourceCreatedAt : null;
      const oldestCreatedAt = rows.length
        ? rows[rows.length - 1].sourceCreatedAt
        : null;
...
async function runSyncCycle(trigger: LastRunState["trigger"]): Promise<void> {
  ...
  const batch = await scrapeRowsFromPage(
    currentPage,
    lookbackStartIso,
    feedItems
  );
